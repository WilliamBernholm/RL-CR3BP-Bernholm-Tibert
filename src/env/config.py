...
def _env_flag(name: str) -> bool:
    """Read a sandbox toggle from the environment.

    Used so guard_fix runs can enable the fix without editing this file, which
    keeps the flag-off default (and therefore the published behaviour) intact.
    """
...
class RunConfig:
    ...
    # The LEARNER's seed -- network initialisation and action sampling. Distinct from
    # train_seed / eval_seed, which seed only the ENVIRONMENT.
    #
    # None (the default) reproduces the historical behaviour: nothing is passed to the
    # PPO constructor, so torch is never seeded and every invocation initialises a
    # different network. That is why no run before 2026-08-07 is individually
    # reproducible -- all 57 V1-vs-V2 runs differ from evaluation 0, not by drifting
    # apart later.
    #
    # Set to an int and it goes to the PPO constructor, which calls SB3's
    # set_random_seed. run_experiment sets it to the run's own --seed when
    # MEX_SEED_LEARNER is truthy. Left None otherwise, so existing results stay
    # comparable and turning it on is an explicit, recorded act.
    ...
    #
    # guard_fix: this is per-BURN, and each overlay is a full 10.4-day ballistic
    # scan (~25k substeps). With a long drift an eval episode has ~5 burns, so the
    # cost is invisible. With a short drift it has hundreds, and a single eval
    # episode takes longer than the whole training run. Set MCC_EVAL_OVERLAYS=0
    # for short-drift runs; the ordinary trajectory plots are unaffected.
    generate_mcc_eval_plot: bool = field(
        default_factory=lambda: os.environ.get("MCC_EVAL_OVERLAYS", "1") != "0")
...
class CurriculumStage:
    ...
    # ---------- initial-state dispersion (the noise probe) ----------
    # A Gaussian perturbation of the state the episode STARTS from, drawn once per
    # reset. ppo_a_* hits the LEO departure state, ppo_b_* hits the post-TLI handoff
    # state; each agent gets the one that matches where its episode begins.
    #
    # Nondimensional CR3BP units -- LU for position, VU for velocity -- because both
    # are added straight to the state vector, which carries those units.
    #
    # Distinct from ppo_b_fixed_state_noise_*, which is gated on ppo_b_use_fixed_index
    # and therefore inert on any stage that samples the scenario library. These two
    # apply on every stage.
    #
    # Zero by default, and every config of record zeroes them explicitly
    # (config_from_txt.NOISE_FIELDS / EXCEPTION 1). Only the noise-probe runs set them.
    ppo_a_initial_state_noise_pos: float = 0.0
    ppo_a_initial_state_noise_vel: float = 0.0
    ppo_b_initial_state_noise_pos: float = 0.0
    ppo_b_initial_state_noise_vel: float = 0.0
...
class CR3BPConfig:
    ...
    # ------------------------------------------------------------------
    # SPEED PORT: use the compiled numba propagation kernels (dynamics.py).
    # Validated bit-equivalent (~1e-12) to the original Python propagation by
    # validate_numba_port.py. Set False to fall back to the pure-Python loops.
    use_numba_propagation: bool = True

    # ------------------------------------------------------------------
    # ABLATION: disable the SMDP (semi-Markov) mechanism.
    # When True: (1) the drift-time action component (tau) is IGNORED and the
    # drift is fixed to the midpoint of the current regime bounds (smdp_fixed_tau
    # in [-1,1]); (2) the per-transition dt_ratio is forced to 1.0, so the
    # time-aware discount gamma**dt_ratio collapses to the standard MDP discount.
    smdp_disabled: bool = False
    smdp_fixed_tau: float = 0.0

    # ------------------------------------------------------------------
    # ORTHOGONAL ABLATION FLAGS (experiment_4 Phase 5)
    # Each removes EXACTLY ONE mechanism so effects can be attributed
    # independently; they may be combined freely. Default = full baseline.
    # (The deprecated `smdp_disabled` above stays only so any earlier compound
    # run reproduces; the runner uses these three instead.)
    # ------------------------------------------------------------------
    # Learned maneuver timing. False -> tau is REMOVED from the action space
    # entirely (no dead dimension) and every inter-decision drift is fixed to
    # `fixed_drift_minutes`. Does NOT touch dt_ratio (see below).
    tau_action_enabled: bool = True
    # Constant inter-decision drift (minutes) when tau_action_enabled is False.
    # None -> midpoint of the current regime's drift bounds.
    fixed_drift_minutes: Optional[float] = None
    # SMDP time-aware discount. False -> dt_ratio forced to 1.0 so gamma**dt_ratio
    # collapses to the standard per-step discount. INDEPENDENT of tau: fixing tau
    # does not make transitions instantaneous, so the discount still sees the true
    # dt_ratio unless this flag is off. (Decoupling these is the whole point.)
    time_aware_discount_enabled: bool = True
    # Temporal memory. False -> the LSTM cell is replaced by a width-matched
    # linear layer (a true feed-forward net, equal params), handled in the policy.
    ...
    # ------------------------------------------------------
    # GUARD FIX (guard_fix sandbox only -- not in the thesis tree)
    #
    # The live post-TLI invalid-orbit test, _check_invalid_post_tli_event Case 1,
    # merged two rules the config below keeps separate: "never got out" (stuck)
    # and "got out, now falling back". Its clause 4 is an OR, so proximity to
    # Earth fires it on its own and the radial-velocity test is never reached.
    # It runs for PPO-MCC only (PPO-TLI short-circuits on tli_only_mode), and it
    # terminated any MCC episode whose first drift was under ~182 min at step 1,
    # with -60, while the trajectory was climbing AWAY from Earth.
    #
    # True  -> Case 1 matches the ballistic evaluator's own semantics:
    #          armed AND vrE <= ballistic_invalid_return_vrE_threshold AND rM far.
    #          Also samples max_rE_seen_post_tli at substep resolution.
    # False -> shipped behaviour, bit-identical to the published runs. DEFAULT.
    # ------------------------------------------------------
    # Set via the GUARD_FIX environment variable so runs can toggle it without
    # editing this file; unset => False => published behaviour.
    invalid_guard_fix_enabled: bool = field(
        default_factory=lambda: _env_flag("GUARD_FIX"))

    # Record one row per decision: t, step, rE, rM, vrE, max_rE_seen, armed,
    # each clause, and whether the guard fired. Read by guard_fix/debug/.
    debug_guard_trace: bool = field(
        default_factory=lambda: _env_flag("GUARD_TRACE"))
    ...
    # Initial-state dispersion, nondimensional (see CR3BPConfig)
    ppo_a_initial_state_noise_pos: float = 0.0
    ppo_a_initial_state_noise_vel: float = 0.0
    ppo_b_initial_state_noise_pos: float = 0.0
    ppo_b_initial_state_noise_vel: float = 0.0
