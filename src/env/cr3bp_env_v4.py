...
import dynamics  # compiled numba propagation kernels (speed-only; validated bit-equivalent)
...
def _adaptive_kernel_params():
    """
    Collect the RUN-derived substep-control constants consumed by
    dynamics.propagate_adaptive_kernel (one lookup per drift instead of one per
    RK4 substep). Mirrors in_fine_integration_region() / rk4_target_substep_nondim()
    / fine_rk4_substep_nondim() exactly, so the compiled kernel reproduces the
    original Python propagation (validated to ~1e-12 by validate_numba_port.py).
    """
    fine_region = float(RUN.fine_substep_region_radius)
    tstar = float(RUN.cr3bp_Tstar_s)
    fine_dt = float(minutes_to_nondim_time(RUN.fine_rk4_substep_minutes))
    x0 = float(RUN.rk4_target_transition_min_minutes)
    x1 = float(RUN.rk4_target_transition_max_minutes)
    if x1 <= x0:
        x1 = x0 + 1.0
    y0 = float(RUN.rk4_substep_target_min_minutes)
    y1 = float(RUN.rk4_substep_target_max_minutes)
    return fine_region, fine_dt, tstar, x0, x1, y0, y1
...
def apply_stage_to_cfg(base_cfg: CR3BPConfig, stage: CurriculumStage) -> CR3BPConfig:
    ...
    cfg.ppo_a_initial_state_noise_pos = float(stage.ppo_a_initial_state_noise_pos)
    cfg.ppo_a_initial_state_noise_vel = float(stage.ppo_a_initial_state_noise_vel)
    cfg.ppo_b_initial_state_noise_pos = float(stage.ppo_b_initial_state_noise_pos)
    cfg.ppo_b_initial_state_noise_vel = float(stage.ppo_b_initial_state_noise_vel)
...
class CR3BPFreeReturnEnv(gym.Env):
    ...
    def __init__(self, cfg: CR3BPConfig, seed: Optional[int] = None, reward_model=None):
        ...
        # ABLATION (tau_action_enabled=False): drop the trailing tau component
        # from the action space entirely, so the policy never emits -- and never
        # receives gradient on -- a dead dimension. Drift is then fixed (see
        # _drift_time_from_tau_masked).
        _tau_dim = 1 if self._tau_action_enabled() else 0
        if self._use_tangential_scalar_action():
            # action = [signed_tangential_dv_raw] (+ tau_raw)
            self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(1 + _tau_dim,), dtype=np.float32)
        else:
            # action = [ax, ay] (+ tau_raw)
            self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(2 + _tau_dim,), dtype=np.float32)
    ...
    def _set_post_tli_initial_condition(self, payload: Dict[str, Any]) -> None:
        ...
        # Initial-state dispersion, once per episode, for EVERY ppo_b mode. Distinct
        # from ppo_b_fixed_state_noise_*, which is gated on ppo_b_use_fixed_index and
        # so is inert in any stage that samples the library (MCC-2 stage 0). The noise
        # probe needs a channel that works on all stages, and the PPO-A mirror of it is
        # _apply_ppo_a_initial_state_noise.
        self._apply_ppo_b_initial_state_noise()
    ...
    def _apply_ppo_a_initial_state_noise(self) -> None:
        """Gaussian dispersion of the PPO-A departure state, once per episode.

        The exact mirror of the PPO-B handoff perturbation (see
        _build_post_tli_state_from_handoff_case): additive, isotropic, applied to the
        nondimensional state after it is built and before the first observation.

        Sigmas are in nondimensional CR3BP units. Zero on every run except the noise
        probe, so this is a no-op for the whole headline + ablation matrix.
        """
        self.applied_ppo_a_noise_pos = 0.0
        self.applied_ppo_a_noise_vel = 0.0

        pos_sigma = float(getattr(self.cfg, "ppo_a_initial_state_noise_pos", 0.0))
        vel_sigma = float(getattr(self.cfg, "ppo_a_initial_state_noise_vel", 0.0))

        if pos_sigma > 0.0:
            self.state[0:2] += self.rng.normal(0.0, pos_sigma, size=2)
            self.applied_ppo_a_noise_pos = pos_sigma

        if vel_sigma > 0.0:
            self.state[2:4] += self.rng.normal(0.0, vel_sigma, size=2)
            self.applied_ppo_a_noise_vel = vel_sigma

    def _apply_ppo_b_initial_state_noise(self) -> None:
        """Gaussian dispersion of the PPO-B handoff state, once per episode.

        Same contract as _apply_ppo_a_initial_state_noise: additive, isotropic,
        nondimensional sigmas, zero on every run except the noise probe. Unlike
        ppo_b_fixed_state_noise_*, this is NOT gated on ppo_b_use_fixed_index, so it
        applies to library-sampled stages too.
        """
        self.applied_ppo_b_noise_pos = 0.0
        self.applied_ppo_b_noise_vel = 0.0

        pos_sigma = float(getattr(self.cfg, "ppo_b_initial_state_noise_pos", 0.0))
        vel_sigma = float(getattr(self.cfg, "ppo_b_initial_state_noise_vel", 0.0))

        if pos_sigma > 0.0:
            self.state[0:2] += self.rng.normal(0.0, pos_sigma, size=2)
            self.applied_ppo_b_noise_pos = pos_sigma

        if vel_sigma > 0.0:
            self.state[2:4] += self.rng.normal(0.0, vel_sigma, size=2)
            self.applied_ppo_b_noise_vel = vel_sigma
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        if trainer_mode == "ppo_a":
            ...
            # Initial-state dispersion, applied ONCE per episode. The PPO-A mirror of
            # the PPO-B handoff perturbation below; both are zero on every run except
            # the noise probe.
            self._apply_ppo_a_initial_state_noise()
    ...
    def _propagate(self, dt_total: float):
        # SPEED PORT: dispatch to the compiled numba kernel (validated bit-equivalent
        # to _propagate_python, ~1e-12, by validate_numba_port.py). Set
        # cfg.use_numba_propagation=False to fall back to the original Python loop.
        if getattr(self.cfg, "use_numba_propagation", True):
            return self._propagate_numba(dt_total)
        return self._propagate_python(dt_total)

    def _propagate_numba(self, dt_total: float):
        dt_total = float(dt_total)
        if dt_total <= 0.0:
            return

        record = bool(self.debug_eval or self.cfg.store_dense_training_traj)
        fine_region, fine_dt, tstar, x0, x1, y0, y1 = _adaptive_kernel_params()

        if record:
            min_step = max(min(fine_dt, (60.0 * max(1e-6, min(y0, y1))) / tstar), 1e-12)
            traj_buf, t_buf = dynamics.record_buffers(int(dt_total / min_step) + 8)
        else:
            traj_buf, t_buf = dynamics.empty_record_buffers()

        (
            x, y, vx, vy, t,
            min_rE, min_rM, min_rE_postflyby, best_corridor_dist, best_rp,
            ev_flyby, ev_corridor, ev_corridor_exit,
            early_code, early_r, n_rec,
        ) = dynamics.propagate_adaptive_kernel(
            float(self.state[0]), float(self.state[1]),
            float(self.state[2]), float(self.state[3]),
            float(self.t), dt_total, float(self.cfg.mu),
            fine_region, fine_dt, tstar, x0, x1, y0, y1,
            float(self.cfg.r_moon_flyby), float(self.cfg.rp_min), float(self.cfg.rp_max),
            float(self.cfg.r_earth_impact), float(self.cfg.r_moon_impact),
            bool(self.flyby_done), bool(self.return_corridor_hit_postflyby),
            float(self.min_rE), float(self.min_rM), float(self.min_rE_postflyby),
            float(self.best_postflyby_corridor_dist), float(self.best_postflyby_rp),
            record, traj_buf, t_buf,
        )

        self.state = np.array([x, y, vx, vy], dtype=np.float64)
        self.t = float(t)
        self.min_rE = float(min_rE)
        self.min_rM = float(min_rM)
        self.min_rE_postflyby = float(min_rE_postflyby)
        self.best_postflyby_corridor_dist = float(best_corridor_dist)
        self.best_postflyby_rp = float(best_rp)
        ...
        if record:
            for i in range(int(n_rec)):
                self.traj.append(traj_buf[i].copy())
                self.t_hist.append(float(t_buf[i]))

        if early_code == dynamics.TERM_EARTH_IMPACT:
            self._early_terminate = ("earth_impact", float(early_r))
        elif early_code == dynamics.TERM_MOON_IMPACT:
            self._early_terminate = ("moon_impact", float(early_r))

    def _propagate_python(self, dt_total: float):
        dt_total = float(dt_total)
        if dt_total <= 0.0:
            return
        ...
        while t_remaining > 0.0:
            ...
            # F2: max_rE_seen_post_tli is a PATH-HISTORY quantity. Shipped code updates
            # it only once per decision (inside the guard), so a short tau aliases it and
            # keeps it artificially small. Sample it here, at substep resolution, so it
            # means what its name says. Gated so the flag-off tree stays bit-identical.
            if self.tli_used and bool(getattr(self.cfg, "invalid_guard_fix_enabled", False)):
                if rE_now > self.max_rE_seen_post_tli:
                    self.max_rE_seen_post_tli = float(rE_now)
                if rE_now >= float(self.cfg.ballistic_invalid_return_arm_rE):
    ...
    def _propagate_copy(self, state: np.ndarray, dt_total: float) -> np.ndarray:
        if getattr(self.cfg, "use_numba_propagation", True):
            return self._propagate_copy_numba(state, dt_total)
        return self._propagate_copy_python(state, dt_total)

    def _propagate_copy_numba(self, state: np.ndarray, dt_total: float) -> np.ndarray:
        dt_total = float(dt_total)
        ...
        fine_region, fine_dt, tstar, x0, x1, y0, y1 = _adaptive_kernel_params()
        traj_buf, t_buf = dynamics.empty_record_buffers()
        out = dynamics.propagate_adaptive_kernel(
            float(s[0]), float(s[1]), float(s[2]), float(s[3]),
            0.0, dt_total, float(self.cfg.mu),
            fine_region, fine_dt, tstar, x0, x1, y0, y1,
            # events neutralized: flyby/corridor/impact radii can never trigger,
            # matching the original _propagate_copy (pure integration, no events).
            -1.0, 0.0, 0.0, -1.0, -1.0,
            False, False,
            np.inf, np.inf, np.inf, np.inf, np.nan,
            False, traj_buf, t_buf,
        )
        return np.array([out[0], out[1], out[2], out[3]], dtype=np.float64)

    def _propagate_copy_python(self, state: np.ndarray, dt_total: float) -> np.ndarray:
        dt_total = float(dt_total)
        s = np.array(state, dtype=np.float64).copy()
    ...
    # ------------------------------------------------------------------
    # SPEED PORT helpers (shared by the numba ballistic evaluators). These
    # reproduce the original inline result-dict / classification / budget logic.
    # ------------------------------------------------------------------
    def _ballistic_result_dict(
        self, min_rM, vrel_at_min_rM, flyby_done, min_rE_postflyby,
        corridor_hit_postflyby, term_reason, success, rE_terminal,
        rM_terminal, left_leo, dv_used_ballistic,
    ):
        corridor_dist = (
            float(dist_to_interval(float(min_rE_postflyby), self.cfg.rp_min, self.cfg.rp_max))
        ...
        return {
            ...
            "left_leo": bool(left_leo),
            "dv_used_ballistic": float(dv_used_ballistic),
        }

    def _classify_ballistic_timeout(self, flyby_done, max_rE_seen, corridor_exit_outward):
        if (
            ...
            return "invalid_preflyby_earth_return", False
        if corridor_exit_outward:
            return "success", True
        return "timeout", False

    def _ballistic_budget_exceeded(self) -> bool:
        return bool(
            self.cfg.terminate_on_dv_budget_exceed
            and (self.reward_model is not None)
            and (float(self.dv_used) > float(self.reward_model.cfg.dv_budget))
        )

    def _build_ballistic_reference_from_tli(self):
        # SPEED PORT: numba kernel by default; Python fallback validated equivalent.
        if getattr(self.cfg, "use_numba_propagation", True):
            return self._build_ballistic_reference_from_tli_numba()
        return self._build_ballistic_reference_from_tli_python()

    def _build_ballistic_reference_from_tli_numba(self):
        self.ballistic_ref_traj = []
        self.ballistic_ref_t_hist = []
        ...
        n_sub = max(1, int(getattr(self.cfg, "integration_substeps", 1)))
        dt_sub = dt / n_sub
        t_end = float(self.cfg.t_max)
        if t_ref >= t_end:
            return
        traj_buf, t_buf = dynamics.record_buffers(int(math.ceil((t_end - t_ref) / dt_sub)) + 4)
        out = dynamics.ballistic_scan_kernel(
            float(s[0]), float(s[1]), float(s[2]), float(s[3]), t_ref,
            t_end, dt_sub, float(self.cfg.mu),
            np.inf,                # r_leo_exit tracking irrelevant here
            False, 0.0, 0.0, 0.0,  # invalid-return detection disabled
            -1.0, 0.0, 0.0,        # flyby/corridor cannot trigger
            float(self.cfg.r_earth_impact), float(self.cfg.r_moon_impact),
            np.inf,                # escape does not stop the reference branch
            False,                 # no dv budget on a ballistic reference
            False, np.inf, False, False, False,
            np.inf, np.nan,
            True, traj_buf, t_buf,
        )
        term_code = int(out[14])
        n_rec = int(out[17])
        for i in range(n_rec):
            self.ballistic_ref_traj.append(traj_buf[i].copy())
            self.ballistic_ref_t_hist.append(float(t_buf[i]))
        if term_code in (dynamics.TERM_EARTH_IMPACT, dynamics.TERM_MOON_IMPACT):
            self.ballistic_terminal_marker_rot = np.array([out[0], out[1]], dtype=np.float64)

    def _build_ballistic_reference_from_tli_python(self):
        self.ballistic_ref_traj = []
        self.ballistic_ref_t_hist = []
    ...
    # ------------------------------------------------------------------
    # ORTHOGONAL ABLATION HELPERS (experiment_4 Phase 5). See config.py.
    # Backward-compatible with the deprecated compound `smdp_disabled`.
    # ------------------------------------------------------------------
    def _tau_action_enabled(self) -> bool:
        """True when tau is PRESENT IN THE ACTION SPACE (orthogonal flag only)."""
        return bool(getattr(self.cfg, "tau_action_enabled", True))

    def _tau_drives_drift(self) -> bool:
        """True when the emitted tau actually determines the drift duration.
        False under tau_action_enabled=False (tau absent) or the deprecated
        smdp_disabled (tau present but ignored)."""
        if getattr(self.cfg, "smdp_disabled", False):
            return False
        return self._tau_action_enabled()

    def _time_aware_discount_enabled(self) -> bool:
        """True when dt_ratio reports the real transition duration. Deliberately
        INDEPENDENT of tau: fixing tau does not make transitions instantaneous."""
        if getattr(self.cfg, "smdp_disabled", False):
            return False
        return bool(getattr(self.cfg, "time_aware_discount_enabled", True))

    def _fixed_drift_nondim(self) -> float:
        """Constant inter-decision drift used when tau is not learned.
        fixed_drift_minutes=None -> midpoint of the CURRENT regime's bounds
        (matches the old smdp_disabled behaviour); an explicit value is clipped
        to the current regime's bounds."""
        dt_min, dt_max = self._current_tau_bounds_nondim()
        fixed_min = getattr(self.cfg, "fixed_drift_minutes", None)
        if fixed_min is None:
            return float(0.5 * (dt_min + dt_max))
        dt_fixed = minutes_to_nondim_time(float(fixed_min))
        return float(np.clip(dt_fixed, dt_min, dt_max))

    def _equivalent_tau_of_fixed_drift(self) -> float:
        """The tau in [-1,1] that WOULD produce the current fixed drift, so
        fixed-tau arms plot on the same axis as learned-tau runs. NaN if the
        regime bounds are degenerate."""
        dt_min, dt_max = self._current_tau_bounds_nondim()
        span = float(dt_max - dt_min)
        if span <= 1e-15:
            return float("nan")
        tau01 = (self._fixed_drift_nondim() - dt_min) / span
        return float(2.0 * float(np.clip(tau01, 0.0, 1.0)) - 1.0)

    def _drift_time_from_tau_masked(self, tau_raw: float) -> float:
        # When tau is not used (tau_action_enabled=False or deprecated
        # smdp_disabled) return the fixed drift; otherwise map tau -> drift.
        if not self._tau_drives_drift():
            return self._fixed_drift_nondim()
    ...
    def _evaluate_ballistic_after_tli(self, state_after_tli: np.ndarray, t_after_tli: float) -> Dict[str, Any]:
        # SPEED PORT: numba kernel by default; Python fallback validated equivalent.
        if getattr(self.cfg, "use_numba_propagation", True):
            return self._evaluate_ballistic_after_tli_numba(state_after_tli, t_after_tli)
        return self._evaluate_ballistic_after_tli_python(state_after_tli, t_after_tli)

    def _evaluate_ballistic_after_tli_numba(self, state_after_tli: np.ndarray, t_after_tli: float) -> Dict[str, Any]:
        s = np.array(state_after_tli, dtype=np.float64).copy()
        self.ballistic_terminal_marker_rot = None
        mu = float(self.cfg.mu)
        rE_pos, _ = earth_moon_positions(mu)
        rE0 = float(np.linalg.norm(s[:2] - rE_pos))
        left_leo0 = bool(self.left_leo or (rE0 > getattr(self, "r_leo_exit", np.inf)))
        dt_sub = float(self.cfg.dt) / max(1, int(self.cfg.integration_substeps))
        ...
        (
            ...
            max_rE_seen, _invalid_armed,
            term_code, rE_term, rM_term, _n_rec,
        ) = dynamics.ballistic_scan_kernel(
            float(s[0]), float(s[1]), float(s[2]), float(s[3]), float(t_after_tli),
            float(self.cfg.t_max), dt_sub, mu,
            float(getattr(self, "r_leo_exit", np.inf)),
            bool(self.cfg.ballistic_invalid_preflyby_return_enabled),
            float(self.cfg.ballistic_invalid_return_arm_rE),
            float(self.cfg.ballistic_invalid_return_vrE_threshold),
            float(self.cfg.ballistic_invalid_return_moon_far_rM),
            float(self.cfg.r_moon_flyby), float(self.cfg.rp_min), float(self.cfg.rp_max),
            float(self.cfg.r_earth_impact), float(self.cfg.r_moon_impact), float(self.cfg.r_escape),
            self._ballistic_budget_exceeded(),
            False, np.inf, False, False, left_leo0,
            np.inf, np.nan,
            False, traj_buf, t_buf,
        )
        self.ballistic_terminal_marker_rot = np.array([x, y], dtype=np.float64)
        if int(term_code) != dynamics.TERM_NONE:
            term_reason = dynamics.TERM_REASON_BY_CODE[int(term_code)]
            success = False
            rE_terminal = float(rE_term)
            rM_terminal = float(rM_term)
        else:
            term_reason, success = self._classify_ballistic_timeout(
                flyby_done, float(max_rE_seen), bool(corridor_exit_outward)
            )
            dxe = x - float(rE_pos[0])
            dxm = x - (1.0 - mu)
            rE_terminal = math.sqrt(dxe * dxe + y * y)
            rM_terminal = math.sqrt(dxm * dxm + y * y)
        return self._ballistic_result_dict(
            min_rM=min_rM, vrel_at_min_rM=vrel_at_min_rM, flyby_done=flyby_done,
            min_rE_postflyby=min_rE_postflyby, corridor_hit_postflyby=corridor_hit,
            term_reason=term_reason, success=success, rE_terminal=rE_terminal,
            rM_terminal=rM_terminal, left_leo=left_leo, dv_used_ballistic=dv_used_ballistic,
        )

    def _evaluate_ballistic_after_tli_python(self, state_after_tli: np.ndarray, t_after_tli: float) -> Dict[str, Any]:
        s = np.array(state_after_tli, dtype=np.float64).copy()
    ...
    def step(self, action: np.ndarray):
        ...
        # ABLATION (tau_action_enabled=False): the action carries one fewer
        # component. tau_raw is set to the NaN sentinel; _drift_time_from_tau_masked
        # ignores it and returns the fixed drift.
        _tau_on = self._tau_action_enabled()
        if self._use_tangential_scalar_action():
            _n_expect = 1 + (1 if _tau_on else 0)
            if action.size != _n_expect:
                raise ValueError(f"Expected {_n_expect}D action (tau_action_enabled={_tau_on}), got shape {action.shape}")
            dv_tan_raw = float(action[0])
            tau_raw = float(action[1]) if _tau_on else float("nan")
            ...
            _n_expect = 2 + (1 if _tau_on else 0)
            if action.size != _n_expect:
                raise ValueError(f"Expected {_n_expect}D action (tau_action_enabled={_tau_on}), got shape {action.shape}")
            a_xy = action[:2]
            tau_raw = float(action[2]) if _tau_on else float("nan")
        ...
        if not self.tli_used:
            ...
            if is_real_tli and (not self.tli_ballistic_reward_given):
                ...
                # When tau is not learned, tau_raw is the NaN sentinel; log the
                # tau EQUIVALENT to the fixed drift so every arm plots on one axis.
                self.tli_tau = (
                    float(tau_raw) if self._tau_action_enabled()
                    else float(self._equivalent_tau_of_fixed_drift())
                )
    ...
    def invalid_preflyby_case1(
        *,
        ...
        """Case 1 of the post-TLI invalid-orbit test, as a pure predicate.

        Kept free of ``self`` so both semantics can be diffed on recorded states
        without running the simulator (see guard_fix/debug/replay_guard.py).

        use_fix=False -- the original, shipped semantics. Clause 4 is an ``or``,
            so ``rE < 0.9*stuck_max_rE`` fires on its own and the radial-velocity
            test is short-circuited away. This terminates trajectories that are
            climbing away from Earth, purely because they have not yet reached
            0.15, which is what censored the short-drift MCC arms.

        use_fix=True -- matches the ballistic evaluator's own implementation of
            the same rule: the trajectory must actually have got outbound
            (``armed``), and must now be genuinely falling back
            (``vrE <= vrE_threshold``), while still far from the Moon.
        """
        if burn_count < 1:
            return False
        if rM <= moon_far_rM:
        ...
        if use_fix:
            return bool(armed and vrE <= vrE_threshold)

        if max_rE_seen >= stuck_max_rE:
            return False
        return bool(rE < 0.9 * stuck_max_rE or vrE <= 0.0)
    ...
    def _check_invalid_post_tli_event(self) -> Optional[str]:
        ...
        use_fix = bool(getattr(self.cfg, "invalid_guard_fix_enabled", False))
        fires = self.invalid_preflyby_case1(
            rE=rE,
            rM=rM,
            vrE=vrE,
            max_rE_seen=float(self.max_rE_seen_post_tli),
            armed=bool(self.invalid_return_armed_episode),
            burn_count=int(self.mcc_burn_count),
            stuck_max_rE=float(self.cfg.ballistic_invalid_stuck_max_rE),
            moon_far_rM=float(self.cfg.ballistic_invalid_return_moon_far_rM),
            vrE_threshold=float(self.cfg.ballistic_invalid_return_vrE_threshold),
            use_fix=use_fix,
        )

        if bool(getattr(self.cfg, "debug_guard_trace", False)):
            self.guard_trace.append((
                float(self.t), int(self.steps), rE, rM, vrE,
                float(self.max_rE_seen_post_tli),
                float(self.invalid_return_armed_episode),
                float(self.mcc_burn_count >= 1),
                float(self.max_rE_seen_post_tli < float(self.cfg.ballistic_invalid_stuck_max_rE)),
                float(rM > float(self.cfg.ballistic_invalid_return_moon_far_rM)),
                float(rE < 0.9 * float(self.cfg.ballistic_invalid_stuck_max_rE)),
                float(vrE <= 0.0),
                float(fires),
            ))
    ...
    def _get_info(self, extra: Optional[Dict[str, Any]] = None):
        ...
        # ABLATION (time_aware_discount_enabled=False, or deprecated smdp_disabled):
        # report dt_ratio=1 so the PPO time-aware discount gamma**dt_ratio collapses
        # to the standard per-step discount gamma. INDEPENDENT of the tau flag.
        if not self._time_aware_discount_enabled():
            info["dt_ratio"] = 1.0
        else:
