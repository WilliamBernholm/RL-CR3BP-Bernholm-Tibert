...
def build_curriculum_ppob():
    ...
    # Aligned to run PPOB_2026-05-08_10-56-47 (thesis MCC run): handoff library
    # + fixed index 65.
    MAIN_LIB = str(
        Path("rough_scenario_classification")
        / "ppob_handoff_states_30min.npz"
    )
    ...
    overrides = {
        "run": {
            # guard_fix: this curriculum-level override used to be a hard True, which
            # stomped the RunConfig default and made MCC_EVAL_OVERLAYS=0 do nothing.
            # The overlay is a full 10.4-day ballistic scan PER BURN in eval episodes:
            # ~5 burns/episode at 3000 min drift (negligible) but ~250 at 60 min and
            # ~1500 at 10 min, where a single eval outlasts the whole training run.
            "generate_mcc_eval_plot": os.environ.get("MCC_EVAL_OVERLAYS", "1") != "0",
        },
        "env": {
            # example:
