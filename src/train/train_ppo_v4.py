...
# Validation rerun: faithful reconstruction of a run's config from its archived
# run_config.txt (noise forced off, optional dv-scale normalization, hard round-trip).
from config_from_txt import build_full_config_from_txt, ConfigMismatchError, MCC_CANONICAL_DV_SCALE
...
def make_new_run_dir(script_path: str, run_label: str = "RUN") -> Path:
    # Validation rerun: when the executor sets VALIDATION_RUN_DIR, all outputs for
    # this job go there (isolated per-job dir), instead of the tree's Saved Policies.
    override = os.environ.get("VALIDATION_RUN_DIR")
    if override:
...
class TrajectoryEvalCallback:
    def __init__(
        ...
        # ------------------------------------------------------------
        # POLICY RETENTION -- keep 3 per run, not one per eval.
        #
        # This callback saves a policy on EVERY eval. Unthrottled that is ~165 zips
        # per run, and the 57-run queue produced 9387 of them totalling 20.8 GB --
        # 77 % of the whole results tree, and enough to fill kraken's disk.
        #
        # The three that carry information:
        #   first          the initial policy, for a before/after comparison
        #   latest success the most recent eval that scored a TRUE 5-point success
        #                  (not the loose milestone -- that one over-reports by ~5x)
        #   last           the final policy, success or not
        # Everything between them is an intermediate nobody replays.
        #
        # Set MEX_KEEP_ALL_POLICIES=1 to restore the old save-everything behaviour.
        # ------------------------------------------------------------
        # DEFAULT OFF, DELIBERATELY. Retaining 3 policies DURING training would delete
        # the checkpoints Table 4 is computed from: score_all scores EVERY checkpoint
        # (196 TLI / 148 MCC denominators) and takes the rate over the final 20 % OF
        # CHECKPOINTS. Keep 3 and that table has nothing to stand on.
        #
        # So the disk problem is solved by ORDERING, not by retention:
        #     train (all checkpoints) -> pack -> score_arms -> prune_policies
        # prune_policies.py applies the same first/latest-success/last rule once the
        # scores are safely extracted. Peak disk is ~23 GB during a 63-run queue,
        # falling to ~0.5 GB after the eval phase prunes.
        #
        # Set MEX_RETAIN_POLICIES=1 to enable in-training retention anyway -- only do
        # that if you do not need Table 4 from the run.
        #
        # (Until 2026-08-05 this was on by default but INERT: the glob below used
        # self.policy_label, which is PPOA/PPOB, while the checkpoints are written with
        # a hardcoded "Model" prefix. It deleted nothing and printed nothing. The glob
        # is fixed now, so the default had to become explicit.)
        self.retain_policies = os.environ.get("MEX_RETAIN_POLICIES", "0") == "1"
        self.keep_all_policies = not self.retain_policies
        self._policy_first: Optional[Path] = None
        self._policy_last_success: Optional[Path] = None
        self._policy_last: Optional[Path] = None
    ...
    def _retain_policies(self, saved: Path, *, had_true5: bool) -> None:
        """Keep first / latest-success / last; delete the intermediates.

        Runs after every eval save. Recomputes the keep set from scratch and removes
        any other policy zip in the run dir, so a resumed or crashed run self-heals
        rather than accumulating.

        'Success' means a TRUE 5-point success in this eval, not the loose milestone
        rate -- the loose one over-reports by roughly 5x, so a policy kept on that
        basis may never have flown a valid free return.
        """
        saved = Path(saved)
        self._policy_last = saved
        if self._policy_first is None:
            self._policy_first = saved
        if had_true5:
            self._policy_last_success = saved

        if self.keep_all_policies:
            return

        keep = {
            p.resolve()
            for p in (self._policy_first, self._policy_last_success, self._policy_last)
            if p is not None
        }
        removed = 0
        # "Model" is hardcoded at the save site (save_eval_model_with_stats is called
        # with policy_label="Model"); self.policy_label is PPOA/PPOB and matched
        # nothing, which is why this silently deleted zero files for a whole queue.
        for path in self.run_dir.glob("Model__*.zip"):
            try:
                ...
                # A locked or already-gone file must never take the training run down.
                pass

        if removed:
            labels = {
                "first": self._policy_first,
                "latest_success": self._policy_last_success,
                "last": self._policy_last,
            }
            kept = ", ".join(f"{k}={v.name}" for k, v in labels.items() if v is not None)
            print(f"[POLICY RETENTION] removed {removed}, keeping {len(keep)}: {kept}")
    ...
    def maybe_eval(self, model, step_count: int):
        ...
        # Validation rerun: honest 5-point ("true") success per episode over the FULL
        # eval batch (not just the single plotted episode), so the success RATE is
        # robust and written every eval, independent of plot thinning.
        ep_true5 = [
            1.0 if episode_success({
                "success": bool(ep.get("success_flag_latched", False)),
                "term_reason": str(ep.get("reason", "")),
            }) else 0.0
            for ep in eval_results
        ]
        true5_rate = safe_mean(ep_true5)
        ...
        # Validation rerun: append this eval's robust metrics to eval_metrics.csv in
        # the run dir, EVERY eval, decoupled from plotting/checkpoint cadence. This is
        # the authoritative source the harvest/bundle use for the true 5-pt success rate.
        try:
            import csv as _csv
            _mpath = self.run_dir / "eval_metrics.csv"
            _new = not _mpath.exists()
            with open(_mpath, "a", newline="", encoding="utf-8") as _mf:
                _mw = _csv.writer(_mf)
                if _new:
                    _mw.writerow(["num_evals", "step", "n_episodes", "true5_rate",
                                  "loose_sr", "mean_reward", "mean_dv"])
                _mw.writerow([int(self.num_evals), int(step_count), len(eval_results),
                              f"{true5_rate:.6f}", f"{sr:.6f}",
                              f"{reward_mean:.6f}", f"{dv_mean:.6f}"])
        except Exception:
            pass
        ...
        _saved_policy = save_eval_model_with_stats(
            model=model,
        ...
        self._retain_policies(_saved_policy, had_true5=any(v > 0.5 for v in ep_true5))

        # ------------------------------------------------------------
        # Keep your plotting/report behavior
        # ------------------------------------------------------------
        # Validation rerun: plot on the (possibly thinned) schedule, but ALWAYS plot
        # an eval that contains a true-5pt success so a free-return trajectory gets
        # captured for the bundle even when plotting is thinned for speed.
        _has_true5 = any(v > 0.5 for v in ep_true5)

        # LEAN ACTION ARCHIVE -- every eval, not just the plotted ones.
        #
        # The branch below writes the full episode archive on
        #     (num_evals % plot_every == 0) or has_true5
        # which is BIASED, not merely sparse: MCC succeeds on nearly every eval and
        # keeps 129 of 147, while TLI succeeds ~12 % of the time and keeps 28 of 195
        # -- and its extras are precisely its SUCCESSFUL evals. Any figure of "what
        # the agent converged to" built on that samples the agent's good days.
        #
        # Deterministic evaluation is what makes the fix cheap: all 16 episodes of an
        # eval are identical (test_preflight.py:121 -- eval_dv_std exactly 0.0,
        # success rate only ever 0 or 1), so episode 0 IS the one the random draw
        # below would return, and taking it consumes no RNG. The draw stays inside
        # the plotting branch at its existing cadence, so the global numpy stream is
        # bit-identical to guard_fix/env_patched.
        try:
            from action_archive import save_action_snapshot

            save_action_snapshot(
                eval_results[0], self.plots_root / "action_snapshots",
                num_evals=int(self.num_evals), step_count=int(step_count))
        except Exception as exc:  # noqa: BLE001
            # A diagnostic must never take down a run that is otherwise training.
            print(f"[ACTIONS] snapshot skipped at step {step_count}: {exc}")

        if (self.num_evals % self.plot_every == 0) or _has_true5:
...
def build_model(
    ...
    # ABLATION (lstm_enabled=False): width-matched feed-forward policy. Identical
    # algorithm, discounting and layer widths; the recurrent cell is replaced by a
    # linear layer, so no information crosses a timestep boundary.
    policy_name = "SquashedMlpLstmPolicy" if lstm_enabled else "SquashedMlpPolicy"
    ...
    model = TimeAwareRecurrentPPOv2(
        ...
        device=RUN.device,
        # The LEARNER's seed. Until 2026-08-07 nothing was passed here, so torch was
        # never seeded and the network initialised differently on every invocation --
        # only the ENV was seeded (train_seed / eval_seed above). That is why no run was
        # individually reproducible. None keeps the old behaviour exactly; SB3 calls
        # set_random_seed only when this is not None.
        seed=RUN.learner_seed,
    )
    print(f"[SEED] learner_seed={RUN.learner_seed!r} "
          f"({'SEEDED -- reproducible' if RUN.learner_seed is not None else 'unseeded'})"
          f"  train_seed={RUN.train_seed}  eval_seed={RUN.eval_seed}")
...
# ==========================================================================
# ABLATION HARNESS  (set by run_ablation.py before calling train())
#   ABLATION_MODE:  "none" | "no_lstm" | "no_smdp"
#   ABLATION_MAX_STEPS: cap on total training steps (e.g. 300_000); None = full
#   ABLATION_PLOTS_TARGET: produce ~this many trajectory-plot evals per run
# Eval frequency is left UNCHANGED; only plotting cadence is thinned.
# ==========================================================================
ABLATION_MODE = "none"
ABLATION_MAX_STEPS = None
ABLATION_PLOTS_TARGET = 6
# Orthogonal ablation modes: "no_lstm" | "no_tau" | "no_time_discount".
# For the tau fixed-drift SWEEP, set a constant drift (minutes) with mode no_tau.
ABLATION_FIXED_DRIFT_MIN = None


def train(training_profile: str, resume_policy_path: Optional[Path] = None,
          config_txt_override: Optional[Path] = None):
    ...
    if config_txt_override is not None:
        # ---- VALIDATION RERUN: rebuild the exact config from the archived
        # run_config.txt (all noise forced off, dv-scale per DV_SCALE_OVERRIDE),
        # instead of the generic Python profile. RUN is mutated in place. The
        # ablation-flag block below still runs -> numba ON + full method.
        csv_guess = Path(config_txt_override).resolve().parents[1] / "parsed_config_summary.csv"
        # dv_scale rule: TLI keeps archived 1.0; MCC uses the MCC max-per-step
        # (fixes MCC-1's nominal 1.0 to match the other MCC runs).
        _dv_ovr = MCC_CANONICAL_DV_SCALE if str(training_profile) == "ppo_mcc" else None
        base_cfg, reward_cfg, curriculum, cfg_report = build_full_config_from_txt(
            config_txt_override, base_cfg, reward_cfg,
            csv_path=(csv_guess if csv_guess.exists() else None),
            dv_scale_override=_dv_ovr,
            strict=True,
        )
        overrides = {"env": {}, "reward": {}, "run": {}}
        _smoke = os.environ.get("VALIDATION_SMOKE_STAGE_STEPS")
        if _smoke:
            _cap = int(_smoke)
            for _st in curriculum:
                _st.timesteps = min(int(_st.timesteps), _cap)
            print(f"[VALIDATION][SMOKE] per-stage steps capped at {_cap}")
        # Plot thinning for throughput + compression: the true-5pt rate is written to
        # eval_metrics.csv EVERY eval regardless, so thinning plots is safe. Evals with
        # a success still always plot (free-return capture).
        _pev = os.environ.get("VALIDATION_PLOT_EVERY_EVALS")
        if _pev:
            RUN.plot_every_evals = max(1, int(_pev))
            print(f"[VALIDATION] plot_every_evals thinned to {RUN.plot_every_evals}")
        # Multi-seed: extra seeds override train_seed only (eval_seed stays archived,
        # so every seed evaluates on the SAME scenario). 'orig' seed = archived =
        # the reproduction target (no override).
        _seed = os.environ.get("VALIDATION_SEED")
        if _seed:
            RUN.train_seed = int(_seed)
            print(f"[VALIDATION] train_seed overridden to {RUN.train_seed} (extra seed; "
                  f"eval_seed stays {RUN.eval_seed})")
        print("\n[VALIDATION] Reconstructed config from archive:")
        print(f"             file            : {cfg_report['config_file']}")
        print(f"             seeds           : train={cfg_report['train_seed']} eval={cfg_report['eval_seed']}")
        print(f"             effective steps : {cfg_report['effective_total_steps']:,} ({cfg_report['n_stages']} stages)")
        print(f"             noise exceptions: {len(cfg_report['exceptions'])} field(s) forced to 0")
        print(f"             round-trip      : OK (0 unintended mismatches)")
    else:
    ...
    # ---- ABLATION: apply env-side flags (persist through apply_stage_to_cfg
    # copies, since these are CR3BPConfig fields) ----
    base_cfg.use_numba_propagation = True  # speed port (validated bit-equivalent)
    # ORTHOGONAL ablation flags: each removes exactly ONE mechanism (config.py).
    ...
    if ABLATION_FIXED_DRIFT_MIN is not None:
        base_cfg.fixed_drift_minutes = float(ABLATION_FIXED_DRIFT_MIN)
    if ABLATION_MODE in ("no_smdp", "no_both"):
        base_cfg.smdp_disabled = True  # DEPRECATED compound, kept for reproducibility
        print("[ABLATION] (deprecated) compound SMDP disabled.")
    print(f"[ABLATION] flags: lstm={base_cfg.lstm_enabled} tau={base_cfg.tau_action_enabled} "
          f"time_discount={base_cfg.time_aware_discount_enabled} "
          f"fixed_drift_min={getattr(base_cfg, 'fixed_drift_minutes', None)}")
    ...
    # ---- ABLATION: tag the run folder and thin the plotting cadence ----
    if ABLATION_MODE != "none":
        run_policy_label = f"{run_policy_label}_ABL_{ABLATION_MODE}"
    _eff_steps = int(ABLATION_MAX_STEPS) if ABLATION_MAX_STEPS else int(total_curriculum_steps)
    _n_evals = max(1, _eff_steps // max(1, int(RUN.eval_interval_steps)))
    RUN.plot_every_evals = max(1, round(_n_evals / max(1, int(ABLATION_PLOTS_TARGET))))
    print(f"[ABLATION] mode={ABLATION_MODE} | max_steps={ABLATION_MAX_STEPS} | "
          f"plots~={ABLATION_PLOTS_TARGET} -> plot_every_evals={RUN.plot_every_evals} "
          f"(eval cadence unchanged at {RUN.eval_interval_steps})")
    ...
    if resume_policy_path is None:
        model = build_model(train_env, run_dir, ent_coef=curriculum[0].entropy_coef,
                            lstm_enabled=bool(getattr(base_cfg, "lstm_enabled", True)))
    ...
    # ABLATION (no_lstm): handled cleanly at build_model time by swapping in the
    # width-matched feed-forward SquashedMlpPolicy (a true memory-free baseline),
    # NOT by the deprecated state-zeroing hack. Nothing to do here.
    ...
    try:
        for si, stage in enumerate(curriculum):
            ...
            while remaining > 0:
                ...
                # ABLATION: clamp so we stop cleanly at the step cap.
                if ABLATION_MAX_STEPS is not None:
                    this = min(this, int(ABLATION_MAX_STEPS) - total_done)
                    if this <= 0:
                        _hit_cap = True
                        break
                ...
                if ABLATION_MAX_STEPS is not None and total_done >= int(ABLATION_MAX_STEPS):
                    print(f"[ABLATION] reached step cap {int(ABLATION_MAX_STEPS):,}; stopping training.")
                    _hit_cap = True
                    break
                ...
                # Validation rerun: machine-readable progress for the status command.
                try:
                    _vr = os.environ.get("VALIDATION_RUN_DIR")
                    if _vr:
                        _pp = Path(_vr) / "progress.json"
                        _tmp = _pp.with_suffix(".json.tmp")
                        with open(_tmp, "w", encoding="utf-8") as _pf:
                            json.dump({
                                "total_done": int(total_done),
                                "total_curriculum_steps": int(total_curriculum_steps),
                                "pct": round(100.0 * total_done / max(1, total_curriculum_steps), 2),
                                "fps": round(float(fps_est), 1),
                                "stage": int(si + 1), "n_stages": int(len(curriculum)),
                                "t": time.time(),
                            }, _pf)
                        _tmp.replace(_pp)
                except Exception:
