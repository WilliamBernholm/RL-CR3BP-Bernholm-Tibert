...
from cr3bp_env_v4 import apply_stage_to_cfg, earth_moon_positions, kms_to_nondim_dv
...
LEGEND_SIZE = 9    # three real entries now have line samples, so it needs less room
...
FLYBY_COLOR = "#D95319"   # MATLAB orange; black collided with the contours
RETURN_COLOR = "#0072BD"  # MATLAB blue; matches the corridor elsewhere
...
def add_circle(ax, xy, radius, color, label=None, fill=False, alpha=1.0, linestyle="-", linewidth=1.5):
    """A filled disc (Earth, Moon) or a ring (flyby bound, return corridor).

    `edgecolor` used to be hardcoded to "none", which meant an UNFILLED circle got
    facecolor "none" and edgecolor "none" -- it drew nothing at all. The flyby bound and
    both return-corridor rings were therefore invisible while still contributing their
    labels to the legend, which is why the legend read as three lines of text with no
    samples beside them. The black rings in the old figure are the zero contour, not
    these.
    """
    ax.add_patch(Circle(
        xy, radius,
